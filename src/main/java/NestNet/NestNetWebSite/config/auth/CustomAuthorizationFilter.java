...
public class CustomAuthorizationFilter extends OncePerRequestFilter {       //http 요청마다 처리하도록 하는 필터
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        log.debug("CustomAuthorizationFilter.class / doFilterInternal :" + servletPath +  ": 엑세스 토큰을 검사");
        ...
        if(!StringUtils.hasText(accessToken)){
            log.debug("CustomAuthorizationFilter.class / doFilterInternal : 엑세스 토큰 없음");
        ...
        if(!nowCreated && redisUtil.hasKey(accessToken)){
            log.debug("CustomAuthorizationFilter.class / doFilterInternal : 블랙리스트에 등록된 엑세스 토큰");
        ...
        if(validatedAccessToken != null){
            ...
            log.debug("CustomAuthorizationFilter.class / doFilterInternal : JWT access 토큰, refresh 토큰 모두 유효하지 않음");
