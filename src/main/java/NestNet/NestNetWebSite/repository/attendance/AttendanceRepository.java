...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    @Query("select a.member, count(a) from Attendance a " +
            ...
            "group by a.member")
    List<Object[]> findWeeklyStatisticsByMember(@Param("startOfWeek") LocalDateTime startOfWeek, @Param("endOfWeek") LocalDateTime endOfWeek);

    // 월간 출석 통계 조회
    @Query("select a.member, count(a) from Attendance a " +
            "where a.time  >=:startDayOfMonth and a.time <=:endDayOfMonth " +
            "group by a.member")
    List<Object[]> findMonthlyStatisticsByMember(@Param("startDayOfMonth") LocalDateTime startOfMonth, @Param("endDayOfMonth") LocalDateTime endOfMonth);
